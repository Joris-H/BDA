...
### fit again
# fit complete model
model_ext_out <- lm(Extr ~(.), data=data_ext_outrm)

# make stepwise selection
model_ext_final <- stepAIC(model_ext_out, direction = "both", trace = F)
...
### fit again
# fit complete model
model_agr_out <- lm(Agr ~(.), data=data_agr_outrm)

# make stepwise selection
model_agr_final <- stepAIC(model_agr_out, direction = "both", trace = F)
...
### fit again
# fit complete model
model_cons_out <- lm(Cons ~(.), data=data_cons_outrm)

# make stepwise selection
model_cons_final <- stepAIC(model_cons_out, direction = "both", trace = F)
...
### fit again
# fit complete model
model_emot_out <- lm(Emot ~(.), data=data_emot_outrm)

# make stepwise selection
model_emot_final <- stepAIC(model_emot_out, direction = "both", trace = F)
...
### fit again
# fit complete model
model_open_out <- lm(Open ~(.), data=data_open_outrm)

# make stepwise selection
model_open_final <- stepAIC(model_open_out, direction = "both", trace = F)
...
write_csv(predictions_unite, "master_predictions_final1")
